...
from database.db import (
    get_user, update_user, find_partner, connect_users,
    disconnect_users, is_premium_active
)
...
async def start_search(message: Message, gender_filter: GenderEnum = None):
    ...
    if user.current_partner_id:
        await message.answer("⚠️ Siz allaqachon chatdasiz.")
        return

    await update_user(user_id, is_searching=True)
    ...
    if partner:
        ...
        await message.bot.send_message(partner.id, text, reply_markup=chat_keyboard(), parse_mode="HTML")
    else:
        await message.answer(
            "🔍 Muloqotchi qidirilmoqda...",
...
async def search_gender(message: Message):
    ...
    if not await is_premium_active(message.from_user.id):
        await message.answer("⭐ Bu funksiya faqat Premium uchun!", reply_markup=main_menu(False))
        return
    # gender qidiruv logikasi qo'shish mumkin keyinroq
    await start_search(message)
